...
description: |
  ...
  manage dashboards, manage dashboard lists, or export observability data. Trigger phrases include "search logs", "tail logs",
  "query metrics", "check Datadog", "find errors", "trace request", "compare errors",
  "what services exist", "log patterns", "CPU usage", "service health", "dashboard",
  "dashboards", "dashboard list", "dashboard lists".
---
...
alias datadog="npx @leoflores/datadog-cli"
```
...
> **Full Reference:** See [references/dashboards.md](references/dashboards.md) for complete documentation with all flags, widget types, and advanced examples.

```bash
datadog dashboards list [--count <n>] [--start <n>] [--shared] [--deleted]
datadog dashboards get --id "<dashboard-id>"
datadog dashboards create --title "<title>" --layout <ordered|free> [options]
datadog dashboards update --id "<dashboard-id>" --title "<title>" --layout <ordered|free> [options]
datadog dashboards delete --id "<dashboard-id>"
```

**All Create/Update Options:**

| Flag | Description |
|------|-------------|
| `--widgets <json>` | Widgets JSON array (or pipe via stdin) |
| `--description <text>` | Dashboard description |
| `--template-variables <json>` | Template variables for dynamic filtering |
| `--notify-list <json>` | User handles to notify on changes |
| `--read-only` | Make dashboard read-only |
...
# With description and template variables
datadog dashboards create \
  --title "Service Dashboard" \
  --layout ordered \
  --description "Production monitoring with environment filtering" \
...
# Update existing dashboard with new description
...
cat widgets.json | datadog dashboards create --title "Complex" --layout ordered --description "From file"
```

### Dashboard Lists

Manage dashboard lists (CRUD) and list membership (items).

```bash
datadog dashboard-lists list
datadog dashboard-lists get --id <list-id>
datadog dashboard-lists create --name "<name>"
datadog dashboard-lists update --id <list-id> --name "<name>"
datadog dashboard-lists delete --id <list-id>

datadog dashboard-lists items --id <list-id>
datadog dashboard-lists add-items --id <list-id> --dashboards '<json>'
datadog dashboard-lists update-items --id <list-id> --dashboards '<json>'
datadog dashboard-lists delete-items --id <list-id> --dashboards '<json>'
```
